...
public class MainActivity : Activity, ISurfaceHolderCallback2
{
	...
	const int ScenePermissionRequest = 1001;
	View surface;

	protected override void OnCreate(Bundle savedInstanceState)
	{
		base.OnCreate(savedInstanceState);
		if (Build.VERSION.SdkInt < BuildVersionCodes.M || CheckSelfPermission(ScenePermission) == Permission.Granted)
			Run();
		else
		{
			SetContentView(Resource.Layout.activity_main);
			RequestPermissions(new[] { ScenePermission }, ScenePermissionRequest);
		}
	}
	...
	public override void OnRequestPermissionsResult(int requestCode, string[] permissions, [GeneratedEnum] Permission[] grantResults)
	{
		...
		if (requestCode == ScenePermissionRequest)
			Run(); // The mesh stepper reports a useful status if access was denied.
	}
